    ...
    try {
        ...
        document.getElementById('app').innerHTML = `
            <div style="text-align:center;padding:80px 20px;color:var(--text-sub);">
                <p style="font-size:2.5rem;opacity:0.4;">⚠️</p>
                <p style="margin-top:12px;">页面加载出现问题，请刷新重试。</p>
                <p style="font-size:0.78rem;margin-top:8px;color:var(--text-muted);">${e.message}</p>
            </div>`;
    }
});
    ...
    try {
        ...
        <section class="hero">
            <div class="hero-inner">
                <div class="hero-left">
                    <div class="hero-eyebrow">✦ 心理学 · 自我成长 · 内心疗愈</div>
                    <h1 class="hero-title">探索内心的<em>旅程</em></h1>
                    <p class="hero-subtitle">
                        在这里，我们用心理学的视角理解自己，用科学的方法改善生活。<br>
                        每一篇文章都是一次心灵的对话，愿你在这里找到属于自己的答案。
                    </p>
                    <div class="hero-cta">
                        <a href="#articles" class="btn btn-light">📖 开始阅读</a>
                        <a href="#about" class="btn btn-ghost">👋 了解作者</a>
                    </div>
                </div>
                <div class="hero-right">
                    ${recentThree.map((a, i) => `
                    <div class="hero-card" data-id="${a.id}">
                        <div class="hero-card-label">${i === 0 ? '最新发布' : '推荐阅读'}</div>
                        <div class="hero-card-title">${a.title}</div>
        ...
        <section class="home-section">
            <div class="container">
                <div class="section-head">
                    ...
                    <a href="#articles" class="section-link">查看全部 →</a>
                </div>
                <div class="articles-grid featured">
                    ${allArticles.slice(0, 5).map((a, i) => createArticleCard(a, i === 0)).join('')}
                </div>
            </div>
        </section>
        ...
        <section class="home-section">
            <div class="container">
                <div class="section-head">
                    ...
                    <a href="#tags" class="section-link">全部标签 →</a>
                </div>
                <div class="hot-tags">
                    ${hotTags.map(([tag, count]) => `
                    <a href="#/tag/${encodeURIComponent(tag)}" class="hot-tag">${tag} <span>(${count})</span></a>
                    `).join('')}
                </div>
            </div>
        </section>`;
        ...
        app.innerHTML = `<div class="container"><p style="padding:40px;color:var(--text-sub);">首页加载失败：${e.message}</p></div>`;
    }
}
...
function renderArticles() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (articles.length === 0) {
            ...
            articlesGrid.innerHTML = articles.map(a => createArticleCard(a)).join('');
            bindArticleCardEvents();
        }
    });
...
function renderArticleDetail(params) {
        ...
        app.innerHTML = `
            <div class="container fade-in">
                <div class="empty-state">
                    <div class="empty-state-icon">😕</div>
                    <p>文章不存在</p>
                    <a href="#articles" class="section-link" style="margin-top:16px;display:inline-block;">← 返回文章列表</a>
                </div>
            </div>`;
        return;
    }
    ...
    // 图片渐变 class
    const imgClass = getImageClass(article.image);

    app.innerHTML = `
        <!-- 文章顶部英雄图 -->
        <div class="article-hero">
            <div class="article-hero-bg ${imgClass}"></div>
            <div class="article-hero-overlay"></div>
            <div class="article-hero-content">
                <div class="article-hero-inner">
                    <div class="article-hero-tags">
                        ${article.tags.map(t => `<span class="article-hero-tag">${t}</span>`).join('')}
                    </div>
        ...
        <div class="article-layout">
            ...
            <article class="article-detail">
                <div class="article-body" id="articleBody">${htmlContent}</div>

                <div class="article-bottom">
                    <div class="share-buttons">
                        <button class="share-btn share-wechat" onclick="shareToWeChat('${article.title}')">💬 微信分享</button>
                        <button class="share-btn share-weibo" onclick="shareToWeibo('${article.title}')">📢 微博分享</button>
                        <button class="share-btn share-link" onclick="copyLink()">🔗 复制链接</button>
                    </div>
                    ...
                    <div class="comments-section">
                        ...
                        <div id="comments-container">
                            <p style="color: var(--text-muted); font-size: 0.875rem;">
                                评论功能需要部署后才能使用。你可以使用
                                <a href="https://disqus.com" target="_blank" style="color: var(--primary);">Disqus</a> 或
                                <a href="https://twikoo.js.org" target="_blank" style="color: var(--primary);">Twikoo</a> 添加评论系统。
                            </p>
                        </div>
                    </div>
                </div>
...
function renderTags() {
    ...
    app.innerHTML = `
        <div class="container fade-in">
            <div class="page-header">
                <div class="page-eyebrow">Tags</div>
...
function renderTagArticles(params) {
    ...
    app.innerHTML = `
        <div class="container fade-in">
            <a href="#tags" class="back-btn">← 返回标签页</a>
            <div class="page-header">
        ...
        <div class="container fade-in">
            <section class="about-section">
                ...
                <div class="about-topics">
                    <h2 class="section-title" style="text-align:center;margin-bottom:0;">📚 主要研究方向</h2>
                ...
                <div class="about-contact">
                    <h2 class="section-title" style="text-align:center;margin-bottom:0.5rem;">📬 与我联系</h2>
                    <p style="text-align:center;color:var(--text-sub);margin-bottom:1.5rem;font-size:0.9rem;">如果你有任何想法、问题或合作意向，欢迎通过以下方式联系我。</p>
                    <div style="display:flex;gap:1rem;justify-content:center;flex-wrap:wrap;">
                        <a href="mailto:28324386@qq.com" class="contact-btn">📧 发送邮件</a>
                        <a href="#articles" class="contact-btn contact-btn-secondary">📖 阅读文章</a>
                    </div>
                </div>
            </section>
        </div>`;
    ...
    return `
        <article class="article-card" data-id="${article.id}">
            <div class="article-image">
                <div class="article-image-bg ${imgClass}"></div>
                ${categoryTag ? `<span class="article-category-badge">${categoryTag}</span>` : ''}
            </div>
...
// 根据 image 字段返回对应的 CSS class
function getImageClass(image) {
    const map = {
        'emotion':             'img-heart',
        'procrastination':     'img-brain',
        'attachment':          'img-relation',
        'mindfulness':         'img-growth',
        'cognitive':           'img-mind',
        'self-compassion':     'img-heart',
        'anxiety':             'img-trauma',
        'sleep':               'img-sleep',
        'communication':       'img-relation',
        'positive-psychology': 'img-growth',
        'trauma':              'img-trauma',
        'hypnosis':            'img-hypnosis',
        'special-education':   'img-special-education',
    };
    return map[image] || 'img-default';
}

// 获取所有标签（去重计数）
function getAllTags() {
    const tagMap = {};
    blogData.articles.forEach(a => {
...
function getArticlesByTag(tag) {
    return blogData.articles.filter(a => a.tags.includes(tag));
}

// 根据 ID 获取文章
function getArticleById(id) {
    return blogData.articles.find(a => String(a.id) === String(id));
}
...
function generateTOC(content) {
    ...
    return headings.map(heading => {
        ...
        const id = title.toLowerCase().replace(/[^\w\s\u4e00-\u9fff]/g, '').replace(/\s+/g, '-');
